...
const CreateFood = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            const response = await axios.post("https://zomato-backend-1-4edy.onrender.com/api/food/reel/upload", formData, {
                withCredentials: true,
            });

            console.log(response.data);
            const meRes = await axios.get("https://zomato-backend-1-4edy.onrender.com/api/auth/food-partner/me", {
                withCredentials: true,
            });
            const meId = meRes?.data?.foodPartner?._id;
            const returnTo = location.state?.returnTo;
            // Go back to the page you came from (e.g. profile), not a separate dashboard
            if (returnTo) {
                navigate(returnTo, { replace: true });
            } else if (meId) {
                navigate(`/profile/${meId}`, { replace: true });
            } else {
            ...
            const msg =
                ...
                err?.message ||
                'Upload failed. Please login again and try.';
            setSubmitError(msg);
        } finally {
